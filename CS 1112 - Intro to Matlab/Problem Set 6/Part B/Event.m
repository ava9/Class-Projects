...
% which it's scheduled.
    ...
    methods
        ...
        % If self cannot be scheduled in that Interval, then t is inf.
            ...
            if isempty(self.available.overlap(possibleInterval))
                t=inf;
            elseif self.available.overlap(possibleInterval).getWidth ...
                    >= self.duration 
                t=max(possibleInterval.left,self.available.left);
            else
                t=inf;
            end
        ...
        % rectangle.  Then, if the event is scheduled, the time during
        % which the event is scheduled is drawn as a colored rectangle,
        % with the color being the linear interpolation from magenta to
        % cyan based on the importance field:  cyan correponds to very
        % important (1) while magenta corresponds to unimportant (0).  Both
        % rectangles are centered on y = self.getId() and should have a
        % height less than 1.  Assume that a figure window already is open
        % and hold is on.
            ...
            plot([self.available.left self.available.right ...
                self.available.right self.available.left ... 
                self.available.left], [self.id-.4 self.id-.4 self.id+.4 ...
                self.id+.4 self.id-.4], 'k','Linewidth',1)
            
            if self.scheduledTime~=-1
                color=self.importance*bestColor + ...
                    (1-self.importance)*worstColor;
                
                endtime=self.scheduledTime+self.duration;
                
                fill([self.scheduledTime endtime endtime...
                    self.scheduledTime],[self.id-.4 self.id-.4 ...
                    self.id+.4 self.id+.4],color)                
            end
